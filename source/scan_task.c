...
char stringBuffer[LINE_LENGTH]; /* sprintf buffer for building strings to send to tft_task */
...
void scan_task(void *arg)
{
    ...
    while (true)
    {
        ...
        sprintf(stringBuffer, "  #   SSID          RSSI Chan");
		xQueueSend(stringQueue, stringBuffer, 100);
...
static void print_scan_result(cy_wcm_scan_result_t *result)
{
    ...
    printf(" %2"PRIu32"   %-12s  %4d  %2d  %02X:%02X:%02X:%02X:%02X:%02X\n",
           num_scan_result, result->SSID,
           result->signal_strength, result->channel, result->BSSID[0], result->BSSID[1],
           result->BSSID[2], result->BSSID[3], result->BSSID[4], result->BSSID[5]);

    /* print formatted scan result to stringBuffer and send to TFT task via queue */
    sprintf(stringBuffer, " %2"PRIu32"   %-12s  %4d  %2d",
            num_scan_result, result->SSID,
